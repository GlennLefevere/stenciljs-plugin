...
val pluginReleaseChannel = properties("pluginVersion").get()
    .split('-')
    .getOrElse(1) { "default" }
    .split('.')
    .first()
...
qodana {
    ...
    resultsPath = layout.buildDirectory.dir("reports/inspections").get().asFile.canonicalPath
...
intellijPlatform {
    ...
    publishing {
        ...
        // The pluginVersion is based on SemVer and supports pre-release labels, like 2.1.7-alpha.3.
        channels = listOf(pluginReleaseChannel)
    }
}

// Configure the UI test IDE with the IntelliJ Platform Gradle Plugin 2.x testing extension.
